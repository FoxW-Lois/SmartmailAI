...
## Objectifs
  Concurrencer les grosses sociétés (GAFAM) et proposer une solution abordable, sécurisée et pérenne pour les TPE/PME. [Et dans le cadre de la mise en public de ce projet, proposer une solution libre d'usage et opensource.]  
...
## Installation utilisateur
  Concernant l'installation côté utilisateur, il est recommandé de suivre la courte documentation *[Comment installer et autoriser un certificat - Windows 10.11 (.NET 9 &+, WinUI 3...).docx](https://github.com/FoxW-Lois/SmartmailAI/tree/main/Documentation)*.  
  Pour récupérer le package d'installation Windows (x64) **.msix**, celui-ci est disponible (avec les autres fichiers du package) au sein de la section *[Releases](https://github.com/FoxW-Lois/SmartmailAI/releases/)* du repository GitHub.  
  Il est également possible d'en générer un soi-même : pour cela il est nécessaire de cloner ce repository et de suivre la documentation *[Comment build un fichier d’installation Windows 10.11 (.NET 9 &+, WinUI 3...).doxc](https://github.com/FoxW-Lois/SmartmailAI/tree/main/Documentation)*.  
...
## Installation (dèv/lancement en débug)
  Afin de lancer le projet en débug, build un package package d'installation, ou bien encore reprendre le développement, il est nécessaire de réaliser ceci :  
  Il faut une machine sous Windows 10/11 (11 x64 bits de préférence), installer ***[Visual Studio 2026 Community](https://visualstudio.microsoft.com/insiders/)***. Puis installer la charge de travail **Développement d'applications WinUI** à l'IDE.  
  L'utilisation de l'extension **Todo Tree** de **Visual Studio Code** est également préférable pour avoir une vue d'ensemble sur les diverses annotations laissées aux développeurs pour une éventuelle évolution du projet.  
...
## Utilisation
  ...
  - Il est possible de changer la langue, le theme, le colorscheme ou encore d'activer la double authentification avec une application mobile Authenticator (Google Authenticator, Microsoft Authenticator, etc...) en passant par la page des **paramètres**.
...
L'utilisateur de l'application va se connecter avec un compte Smartmail, et utiliser une application mobile Authenticator (Google Authenticator, Microsoft Authenticator, etc...) pour y activer la double authentification. Ensuite lorsqu'il va ajouter une adresse email, l'utilisateur utilisera un serveur SMTP ou l'API Google pour intégrer sa boite mail et ses mails correspondant dans l'application. Les mails de l'utilisateur seront ensuite enregistrés dans la base de données locale SQLite.

***[Dans un contexte de déploiement avec un serveur de license]*** Pour qu'un utilisateur puisse se connecter, il faut qu'une licence soit disponible, et ces informations concernant la licence seront enregistrées dans une base de données externe MariaDB. Celle-ci peut permettre de bloquer à distance l'utilisation de l'application si par exemple un client ne renouvelle pas sa licence, ou bien récupère le package d'installation sans en avoir payé une.
...
## Sécurité et RGPD
  ...
  - La clé de chiffrement et la session (permettant de rester connecté un certain temps à l'application après fermeture) sont toutes deux chiffrées et stockées de manière sécurisé via le Windows DPAPI (impossible de déchiffrer si l'utilisateur ayant émis la clé et la session n'a pas déverrouillé sa machine avec la bonne session utilisateur)
  - Le modèle d'IA (LLM) Mistral tourne purement en local, ce qui signifie qu'aucune donnée n'est envoyée ou stockée sur un serveur tiers. De plus le prompt-system bride suffisamment le modèle afin qu'il ne dévoile aucune donnée autre que celles fournies par l'utilisateur.

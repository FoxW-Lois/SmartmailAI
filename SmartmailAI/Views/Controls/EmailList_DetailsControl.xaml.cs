...
{
	public EmailList_DetailsViewModel ViewModel { get; }
	private string? _lastRenderedEmail;

	public EmailList_DetailsControl()
	{
		ViewModel = Ioc.Default.GetRequiredService<EmailList_DetailsViewModel>();
	...
	public static readonly DependencyProperty DetailsListMenuItem_EmailProperty = DependencyProperty.Register("DetailsListMenuItem_Email",
		typeof(Email), typeof(EmailList_DetailsControl), new PropertyMetadata(null, OnDetailsListMenuItemPropertyChanged));

	private static void OnDetailsListMenuItemPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
	{
		if (d is EmailList_DetailsControl control && e.NewValue is Email email)
		{
